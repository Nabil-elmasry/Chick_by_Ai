...
st.title("✨ 🏎️ صفحة تدريب النموذج على بيانات الأعطال (حفظ يدوي بالكامل)")
...
if sensor_with_id and carset_with_id:
    ...
    if st.button("🔗 دمج الملفين بناءً على record_id"):
        try:
            ...
            st.markdown(convert_df_to_download_link(merged_df, "merged_data.csv"), unsafe_allow_html=True)
...
if merged_upload:
    ...
    if st.button("🚀 ابدأ التدريب"):
        try:
            ...
            href = f'<a href="data:file/txt;base64,{b64_report}" download="model_results.txt">⬇️ اضغط هنا لتحميل تقرير التقييم</a>'
